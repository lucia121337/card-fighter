...
DROP_CORP = {67, 61}  # 67: '이벤트 등록용' (팝업구조 TEST 등) / 61: '지난 이벤트' 모아보기 placeholder(카드사당 1개, 실제 진행중 이벤트 아님)
...
BRAND = [("알파벳", "현대카드"), ("ALPHABET", "현대카드"), ("the Green", "현대카드"), ("the Pink", "현대카드"),
         ...
         ("카드의정석", "우리카드"),
         # "굿데이"는 예전엔 우리카드 전용명이라 여기 있었으나, KB국민카드도 "KB국민 굿데이 카드"를 출시해서
         # 오분류(우리카드로 잘못 복원) 발생 — 2026-08-01 제거.
...
def main():
    ...
    for e in events:
        if e.get("corp_idx") in DROP_CORP:
            dropped += 1
            continue
        # "카드사" 타입은 상시 제휴 할인 등 캐시백 프로모션이 아닌 항목 — 카드고릴라 자체 캐시백 페이지도 "캐시백" 타입만 노출.
        if e.get("type_txt") != "캐시백":
            dropped += 1
            continue
