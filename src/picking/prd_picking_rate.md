...
3) 실질 사용 금액 도출:
- 실적 포함 항목 필요 결제액 합계(`totalRequiredSum`)와 실적 제외(`isExcluded === true`) 항목 필요 결제액 합계(`excludedRequiredSum`)를 분리 계산.
- 실질 필요 사용 금액 = `Math.max(선택한 전월실적 기준액, 실적 포함 역산 필요 결제액 합계) + 실적 제외 역산 필요 결제액 합계`
- (단, '실질 필요 사용 금액'이 0이거나 계산 불가능한 경우 피킹률 0%로 안전 Fallback 처리)
